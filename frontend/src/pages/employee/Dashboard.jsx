...
export const EmployeeDashboard = () => {
  ...
  // Use React Query hook for data fetching and mutations
  const {
    todayStatus,
    isLoading,
    error,
  ...
  const handleCheckIn = (tasks) => {
    checkIn(tasks, {
      onSuccess: () => setShowCheckInModal(false),
    });
  };

  const handleCheckOut = (attendanceId, tasks) => {
    checkOut({ attendanceId, tasks }, {
        ...
        <div className="flex items-center justify-between">
          ...
          {isCheckedIn ? (
            <Button
              ...
              disabled={!currentSession?.id || isCheckingOut}
            >
              <StopCircle size={20} className={isCheckingOut ? 'animate-spin' : ''} />
            ...
            >
              <PlayCircle size={20} className={isCheckingIn ? 'animate-spin' : ''} />
